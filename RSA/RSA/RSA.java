...
// END GENERATED CODE
	 	...
	 	if(sInputP.length() ==  0) 
			...
			BigInteger Q=new BigInteger(sInputQ);
 			...
 			if (!P.isProbablePrime(10))
				...
				block =0;
				BigInteger Power = new BigInteger("27");
	...
	popup("Invalid Plain Text, Only Character allowed");
		...
		BigInteger valueTwentySix = new BigInteger("27");
												// adding the values in each block	
												// changing the 27 base to base 10		
		...
		int k =0;
		BigInteger itemp;
														// changing from base 10 to based 27
		for (int i =0; i<no_of_blocks;i++)
		{
			itemp = plainBlocks[i];
	...
	if (chr ==  'a') return 1;
	else if ( chr == 'b' ) return 2;
	else if ( chr == 'c' ) return 3;
	else if ( chr == 'd' ) return 4;
	else if ( chr == 'e' ) return 5;
	else if ( chr == 'f' ) return 6;
	else if ( chr == 'g' ) return 7;
	else if ( chr == 'h' ) return 8;
	else if ( chr == 'i' ) return 9;
	else if ( chr == 'j' ) return 10;
	else if ( chr == 'k' ) return 11;
	else if ( chr == 'l' ) return 12;
	else if ( chr == 'm' ) return 13;
	else if ( chr == 'n' ) return 14;
	else if ( chr == 'o' ) return 15;
	else if ( chr == 'p' ) return 16;
	else if ( chr == 'q' ) return 17;
	else if ( chr == 'r' ) return 18;
	else if ( chr == 's' ) return 19;
	else if ( chr == 't' ) return 20;
	else if ( chr == 'u' ) return 21;
	else if ( chr == 'v' ) return 22;
	else if ( chr == 'w' ) return 23;
	else if ( chr == 'x' ) return 24;
	else if ( chr == 'y' ) return 25;
	else if ( chr == 'z' ) return 26;
	else if ( chr == '|' ) return 0; // YOU NEED A 0 VALUE IN THE ALPHABET!!!!
	else  return -1;
	}
	...
	public void encryptDetail()
	{
		...
		OutString = OutString +cPlain[0]+": "+iPlain[0] +"* 27 ^ 0 = "+iPlain[0]*(27^0) +"\n";
		for(int i=1;i<block;i++)
		{	
			
			if (iPlain[i]>9)
				OutString = OutString+ "\t+ " +cPlain[i]+": "+ iPlain[i]+"* 27 ^ "+i+" = "+ (valueTwentySix.pow(i)).multiply(BigInteger.valueOf(iPlain[i]))+"\n";
			else
				OutString = OutString+ "\t+ " +cPlain[i]+":  "+ iPlain[i]+"* 27 ^ "+i+" = "+(valueTwentySix.pow(i)).multiply(BigInteger.valueOf(iPlain[i]))+"\n";
		}
		OutString = OutString+"\t\t-------------------------------------"; 
		...
		boolean bb;
			...
			while(bb = itemp.compareTo(BigInteger.valueOf(0))>0)
				...
				OutString2 =OutString2+"\n\t27| "+itemp+"     - "+OutPlain+" - " + decode(OutPlain);
				OutString2 = OutString2 +"\n\t    ";
		...
		details = "--------------------------\n"+
                 ...
                 "\n--------------------------"+
		          ...
		          "\n\tfloor[ln("+(N.subtract(BigInteger.valueOf(1))).toString()+"/ln(27)] ="+Integer.toString(block)+
		...
		public void decryptDetail()
	{
		...
		OutString = OutString +cEncrypt[0]+": "+iEncrypt[0] +"* 27 ^ 0= "+iEncrypt[0]*(27^0) +"\n";;
		for(int i=1;i<block+1;i++)
		{
		
		if (iPlain[i]>9)
				OutString = OutString+ "\t+ " +cEncrypt[i]+": "+ iEncrypt[i]+"* 27 ^ "+i+" = "+ (valueTwentySix.pow(i)).multiply(BigInteger.valueOf(iEncrypt[i]))+"\n";
			else
			OutString = OutString+ "\t+ " +cEncrypt[i]+":  "+ iEncrypt[i]+"* 27 ^ "+i+" = "+(valueTwentySix.pow(i)).multiply(BigInteger.valueOf(iEncrypt[i]))+"\n";
		}
		OutString = OutString+"\t\t-------------------------------------"; 
		...
		details = "--------------------------\n"+
                 ...
                 "\n--------------------------"+
		          "\n\nCalculate the block size" +
		          "\n\tceling[ln("+(N.subtract(BigInteger.valueOf(1))).toString()+"/ln(27)] ="+Integer.toString(block+1)+
		          "\n\nConvert the first block in to number form"+
		          "\n"+OutString+
